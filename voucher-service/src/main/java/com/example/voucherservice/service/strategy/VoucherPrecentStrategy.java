...
private final VoucherRepository voucherRepository;
  ...
  public void processApprovalDetail(VoucherDetailEntity detail) {
    BigDecimal maxDiscount = detail.getMaxDiscount();
    BigDecimal discountValue = detail.getDiscountValue();
    if (maxDiscount == null || discountValue == null
        || discountValue.compareTo(BigDecimal.ZERO) <= 0
        || discountValue.compareTo(BigDecimal.valueOf(100)) > 0) {
      throw BaseException.builder()
          .httpStatus(HttpStatus.BAD_REQUEST)
          .errorCode("INVALID_PERCENT_DETAIL")
  ...
  public void processExcelRequest(VoucherRequestEntity requestEntity,
    ...
    log.info("Start processing PERCENT excel request: {}", requestId);
    ...
    while (true) {
      ...
      for (VoucherDetailEntity detail : batch) {
        try {
          ...
          if (detail.getDiscountValue().compareTo(BigDecimal.valueOf(100)) > 0) {
            throw BaseException.builder().httpStatus(HttpStatus.BAD_REQUEST)
                .errorCode("INVALID_PERCENT_VALUE")
                .description("Discount percentage must not exceed 100").build();
          }
      ...
      log.info("PERCENT batch #{}: {} items, failed: {}, totalProcessed: {}",
